...
import utilidades.EventosTableModel;
import logica.Evento;
import logica.Universidad;

public class VerEventos extends JDialog {
	...
	private JTable getTable() {
		if (table == null) {
			...
			table.setBackground(Colores.getAzulbarra());
			table.setForeground(Colores.getBlancuzo());
			table.setFont(new Font("Tahoma", Font.PLAIN, 17));
			table.setGridColor(Colores.getVerde());
		//	table.setIntercellSpacing(new Dimension(40,50));
	...
	public void cargarEventos(){
		Evento[] eventos = new Evento[Universidad.getUniversidad().getEventos().size()];
		int i = 0;
		for (Evento c : Universidad.getUniversidad().getEventos()){
			eventos[i] = c;	
			i++;
		}
		EventosTableModel model = new EventosTableModel(eventos);
	...
	public void cargarTablaconEventos()
	{
		ArrayList<Evento> eventos= Universidad.getUniversidad().getEventos();
		Evento[] arreglo = new Evento[eventos.size()];
		
		for(int i=0;i<eventos.size();i++)
		{
			arreglo[i]=eventos.get(i);
		}
		EventosTableModel modelo= new EventosTableModel(arreglo);
